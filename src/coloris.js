(function (window, document, Math) {
  ...
  hueSlider, hueMarker, alphaSlider, alphaMarker, currentEl, currentFormat, oldColor, newColor;
  ...
  function bindFields(selector) {
    ...
    function showPicker(rectangle, color)
    {
        oldColor = color;
        currentFormat = getColorFormatFromStr(oldColor);
        picker.classList.add('clr-open');

        updatePickerPositionRoutine(rectangle);
        setColorFromStr(oldColor);
    ...
    //Update the color picker's position and the color gradient's offset
    function updatePickerPositionRoutine(rectangle)
    {
        ...
        if (!settings.inline)
        {
            ...
            var top = scrollY + rectangle.y + rectangle.height + settings.margin;
            ...
            if (parent)
            {
                ...
                {
                    top -= rectangle.height + pickerHeight + settings.margin * 2;
                    reposition.top = true;
                }
            ...
            else
            {
                ...
                {
                    top = scrollY + rectangle.y - pickerHeight - settings.margin;
                    reposition.top = true;
                }
            }
    ...
    {
        updatePickerPositionRoutine(currentEl.getBoundingClientRect());
    }
    ...
    // If revert is true, revert the color to the original value.
    function closePicker(revert)
    {
        // Revert the color to the original value if needed
        if (revert && oldColor !== newColor)
        {
            newColor = oldColor;
    ...
    {
        newColor = color !== undefined ? color : colorValue.value;
